...
- **Services**: `src/Services/*`
  ...
  - **PlotsRestService** / **PlotDatasetMapper**: `GET /wp-json/contextualwp-housebuilder/v1/plots`: read-only, authenticated (default capability `edit_posts`, filterable), **published** plot-like posts; public monitoring fields only. Query `page` and `per_page` (default **500**, max **500**); optional **`limit`** uses the same cap and applies when **`per_page` is not present** in the query string (if both are sent, `per_page` wins). Filter **`contextualwp_housebuilder_plot_meta_key_candidates`** supplies ordered meta key candidates per logical field for site-specific ACF/meta naming. Response objects include `id`, `wp_id`, `title`, `status`, `price`, `bedrooms`, `development`, `house_type`, `url`, `last_updated`, **`last_modified_by`** (safe display label for who last modified the plot; **`null`** when unavailable), and **asset completeness signals** where detectable: **`has_floor_plan`**, **`floor_plan_required`**, **`floor_plan_completeness_status`**, plus development intro media signals (**`has_intro_video`**, **`has_intro_image`**, **`intro_media_type`**, **`intro_media_completeness_status`**) when the plot links to a development post. Completeness values are **`present`**, **`missing`**, or **`unknown`**, signals for downstream monitoring tools (for example Contextual Console), not editor-side validation. **`development`** resolves the assigned linked development post **title** even when that development is **draft** (trash/auto-draft still omitted); **`house_type`** linked titles remain **publish-only**.
...
**Current focus areas** for this pack: **relationships** (manifest edges and evidence), **schema interpretation** (post types and taxonomies), **ACF semantics** (field-level tags), **plot dataset REST** (monitoring exports for authenticated clients), **asset completeness signals** (presence/missing/unknown for floor plans and development intro media), and **broader structural detection** (registration metadata and naming signals, not values).
...
- **`contextualwp_housebuilder_plot_floor_plan_required`**: `( ?bool $required, \WP_Post $plot, array $metaKeys )` returns `true`, `false`, or `null`.
- **`contextualwp_housebuilder_development_intro_media_required`**: `( ?bool $required, \WP_Post $development, int $developmentId )` returns `true`, `false`, or `null`.
- **`contextualwp_housebuilder_plot_completeness_signals`**: adjust the plot signal array before it is merged into the REST row.
- **`contextualwp_housebuilder_development_completeness_signals`**: adjust development intro signals before they are merged into the REST row.

Meta key candidates: extend **`contextualwp_housebuilder_plot_meta_key_candidates`** (logical key **`floor_plan`**) and **`contextualwp_housebuilder_development_meta_key_candidates`** (logical keys **`intro_video`**, **`intro_image`**).
