...
## Status: 0.4.8 (asset completeness detection accuracy fix; 0.4.7 default completeness required rules; 0.4.6 plot REST asset completeness signals; 0.4.5 plots REST draft linked **development** title for monitoring; 0.4.4 plots REST `last_modified_by`; min ContextualWP core 1.3.3; 0.4.3 plots REST `status` meta resolution; plot dataset REST + `limit` / `per_page` alignment; Composer-managed autoload patch is 0.3.1; structural detection pass is 0.3.0)
...
- **`PlotsRestService`** / **`PlotDatasetMapper`** (0.4.0, **`last_modified_by`** 0.4.4, **draft development title** 0.4.5, **asset completeness signals** 0.4.6, **default required rules** 0.4.7): `GET /wp-json/contextualwp-housebuilder/v1/plots` - read-only list of **published** plot-like posts; default `edit_posts` permission (filter `contextualwp_housebuilder_rest_plots_capability`); `page` / `per_page` (default **500**, cap **500**); optional query **`limit`** (same cap) when `per_page` is omitted from the query; filter `contextualwp_housebuilder_plot_meta_key_candidates` for ordered meta key resolution; fields `id`, `wp_id`, `title`, `status`, `price`, `bedrooms`, `development`, `house_type`, `url`, `last_updated`, **`last_modified_by`** (safe `display_name` label via ContextualWP core; `null` when unavailable), and **asset completeness signals** (`has_floor_plan`, `floor_plan_required`, `floor_plan_completeness_status`, plus development intro media fields when a development is linked). Default required rules (0.4.7) infer marketing-ready plot floor-plan requirement and published-development intro-media requirement before filters; completeness statuses remain `present` / `missing` / `unknown` only. **`development`** includes assigned linked development **title** when that post is draft/unpublished (not trash/auto-draft); **`house_type`** linked titles remain publish-only. **Detection accuracy (0.4.8)**: presence detection recognises more valid stored shapes so existing assets are not falsely reported as `missing`. Video-labelled fields treat any present value as a video reference (bare IDs, self-hosted URLs, embed/oEmbed values, attachment IDs, nested ACF arrays); intro image values recognise URL, attachment ID, `src`, and ACF image array shapes; floor plans stored in `acf/plot-floorplans` Gutenberg block data in `post_content` are detected via flattened repeater keys (for example `floors_0_floorplan`), filterable through `contextualwp_housebuilder_floor_plan_block_names` and `contextualwp_housebuilder_floor_plan_block_field_pattern`, with existing post-meta detection preserved. This was a detection accuracy fix only: the status enum, requirement defaults, and linked development resolution were not changed. Requires ContextualWP core **1.3.3+**. Pairs with **Contextual Console v1.2.0** for actionable `missing` warnings when site data and filters allow.
...
- Full WordPress bootstrap / integration tests for REST routes in this repo (only focused **`PlotsRestQueryLimits`** unit tests for plots pagination; run `composer test`).
