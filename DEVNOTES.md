...
## Status: 0.4.5 (plots REST draft linked **development** title for monitoring; 0.4.4 plots REST `last_modified_by`; min ContextualWP core 1.3.3; 0.4.3 plots REST `status` meta resolution; plot dataset REST + `limit` / `per_page` alignment; Composer-managed autoload patch is 0.3.1; structural detection pass is 0.3.0)
